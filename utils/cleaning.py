...
def time_to_seconds(time_col):
    ...
    # Pull out the days number if there is one (e.g. "2d" -> 2), else 0
    days = F.regexp_extract(time_col, r"(\d+)d", 1)
    days = F.when(days == "", 0).otherwise(days.cast("int"))

    # Grab the HH:MM:SS part and split it
...
def split_performance(df):
    ...
    # km or mi means it's a distance value
    df = df.withColumn(
        "perf_distance",
        F.when(
    ...
    # h means it's a time value - convert to seconds
    df = df.withColumn(
        "perf_seconds",
...
def add_validity_flag(df):
    # Lab rule: a km/mi event should have a time result,
    # and an h event should have a distance result. Anything else is invalid.
    is_valid = (
...
def clean_rows(df):
    # Keep only valid rows (this also drops the NULLs and the d-events)
    df = df.filter(F.col("is_valid") == True)

    # Filter on age at the event instead of birth year, since events span 1798-2022.
    # A runner should realistically be between 5 and 100 years old at race time.
    ...
    # Country codes were mixed case (ACT, swe) - make them all uppercase
